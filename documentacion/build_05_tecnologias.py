...
stack_section("Backend", [
    ["Node.js + Express", "API REST pura (`server.js`), sin renderizado de HTML.",
     "Simplicidad y control total sobre el ciclo de vida de cada request; separación estricta backend/frontend."],
    ["ES Modules nativos", "Todo el backend usa `import`/`export` sin transpilador.",
    ...
    ["Persistencia en JSON (`jsonStore.js`)", "Colecciones de empresas, outbox de correo/WhatsApp y oportunidades ya notificadas.",
...
stack_section("Frontend", [
    ["Next.js 16 (App Router, Turbopack)", "Único frontend activo (`web/`), Server Components por defecto.",
    ...
    ["TypeScript", "Todo el código de `web/` está tipado.", "Los contratos de datos entre backend y frontend (Expediente, ContractTracking, etc.) se comparten como tipos explícitos, reduciendo errores de integración."],
    ...
    ["Zod", "Validación de entrada en cada route handler de `app/api/**`.", "Valida y tipa la entrada del lado del servidor antes de reenviarla al backend Express."],
])
...
stack_section("Patrones de arquitectura", [
    ["Puertos y adaptadores (hexagonal)", "`GovDataSource`, `EmailPort`, `WhatsAppPort` como interfaces; adaptadores reales y simulados intercambiables.",
    ...
    ["Composición central de dependencias", "`dataSource.js` como único punto donde se decide y envuelve la fuente de datos activa.",
...
stack_section("Herramientas de desarrollo", [
    ["ESLint", "Linting del código de `web/`.", "Consistencia de estilo y detección temprana de errores."],
    ["Git / GitHub", "Control de versiones del proyecto.", "Estándar de la industria."],
    ["npm", "Gestor de paquetes para ambos proyectos (`/` y `/web`).", "Cada proyecto mantiene su propio `package.json` y ciclo de vida independiente."],
])
