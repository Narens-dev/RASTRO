...
## ADR-0018 — RASTRO adoption: upstream API proxy, brand tokens, no OS dark mode

- **Status:** Accepted
- **Date:** 2026-08-16

**Context.** This starter was cloned into `web/` inside an existing project,
**RASTRO** — a Node/Express backend (repo root `src/`, `server.js`) that already
implements the full product: a hexagonal `GovDataSource` port with a real Croma
MCP adapter and a mock fallback, a 6-source risk-evidence engine
(`scoreEngine.js`), contract tracking, a pyme opportunities module, and an
optional Claude-powered summary. That backend is solid and already tested — the
ask was to rebuild only the **frontend** on this starter, not re-implement the
business logic in Next.js.

**Decision.**
1. **Two-app layout, one repo.** `web/` is a self-contained Next.js app with its
   own `package.json`/`node_modules`, dev server on **port 3100** (the Express
   API keeps port 3000, set via `PORT`). Nothing in `web/` reimplements RASTRO's
   business logic.
2. **`app/api/**` route handlers are thin proxies, not reimplementations.**
   Hard rule #9 requires external calls to run server-side in `route.ts` — so
   each RASTRO endpoint (`/api/search`, `/api/entity/[docType]/[doc]`,
   `.../summary`, `/api/contract/[contractId]`, `/api/opportunities`,
   `/api/meta`) gets a matching Next.js route that validates input with `zod`,
   forwards to the Express API via a new shared upstream client
   (`src/lib/rastro-api.ts` → `fetchRastroApi()`, server-only, reads
   `RASTRO_API_URL` from `src/env.ts`), and returns the body through `handle()`.
   This satisfies "the browser only calls same-origin `/api/*`" without
   duplicating scoreEngine/opportunities/contractTracking logic in TypeScript.
3. **Server Components fetch render-time data directly — no self-proxy hop.**
   Per api-architecture.md's own "Render-time data is fetched in Server
   Components instead," `views/expediente.tsx` and `views/contrato.tsx` are
   `async` Server Components that call `fetchRastroApi()` **directly** (not
   through their own `/api/*` route) for the page's primary data — full SSR, no
   client loading spinner for the main content. The `/api/*` routes exist for
   genuine client-side calls after mount: the home search box, the AI summary
   (fetched separately so a slow LLM call never blocks the expediente), the
   footer's live/degraded badge, and the opportunities filter panel (every
   filter change refetches).
4. **Brand tokens replace the starter's placeholder palette**, following the
   three-tier convention (ADR-0015) exactly — primitives sourced from
   `RASTRO-documento-de-proyecto.md` §5 (`--raw-color-emerald-700: #046a38`,
   `--raw-color-gold-600: #c9a227`, `--raw-color-ink-950: #08090a`, …), semantic
   roles (`--action-primary`, `--accent-gold`, and three **evidence** tokens —
   `--evidence-high/-none/-clean` with matching `-surface` variants — mirroring
   the product's non-negotiable alto/sin-hallazgo/limpio levels), and `@theme
   inline` bindings. Fonts swapped from Onest to **Manrope** (`--font-display`,
   headings) + **Inter** (`--font-sans`, body) to match the existing brand.
5. **No `prefers-color-scheme` dark mode.** RASTRO is a dual-tone product by
   design — dark hero/nav/footer, light body — not a user-togglable theme, so
   `--background-inverse` / `--foreground-inverse` are separate Tier-2 tokens
   that sections opt into explicitly (`bg-background-inverse
   text-foreground-inverse`), rather than a `prefers-color-scheme` override on
   `--background`/`--foreground`.
6. **`<Cookie>` not mounted.** RASTRO sets no cookies and has no
   analytics/marketing tracking to consent to — mounting the starter's cookie
   banner would present a consent choice for something that doesn't happen. The
   component files are left in place (harmless, unmounted) rather than deleted,
   since removing shared starter infrastructure wasn't asked for.
7. **New Tier-3 token** `--container-page: 73.75rem` (`max-w-page`) — the page
   body max-width is identical across the nav, footer, and all four views, which
   is exactly ADR-0015's bar for a component token ("shared across components
   that cannot import each other").
8. **One `@layer components` addition**, `.hero-glow` — a three-stop
   `color-mix()` radial-gradient composition for the home hero background.
   Kept out of a Tailwind arbitrary-value utility per ADR-0012's own reasoning
   (too complex to stay "short and readable" in `className`) and out of a React
   component (purely decorative, no markup/props/structure).
9. **Loading-state motion stays instant, not spring-animated, in two spots** —
   the evidence-detail expand/collapse toggle and the AI-summary loading text.
   `@react-spring/web` cannot animate to a measured `height: auto` without an
   extra resize-observer hook not worth adding for a secondary disclosure
   toggle, and a three-dot loading pulse is exactly a `@keyframes` shape (tried
   `animate-bounce` first — a Tailwind default keyframe utility — caught by
   hard rule #1 and replaced with static text). Both are deliberate "no motion"
   rather than a rule violation, not an oversight.

**Consequences.** RASTRO's backend is untouched and remains the single source of
truth for business logic; `web/` is purely presentational plus a thin,
zod-validated proxy layer. A reviewer auditing "does this call a third party
from the browser" only ever finds same-origin `/api/*` calls, same as any other
project from this starter — the third party (Croma) is two hops away (Express →
Croma), not zero, but the browser-facing contract is identical. First
server-only env var: `RASTRO_API_URL` (default `http://localhost:3000`).
