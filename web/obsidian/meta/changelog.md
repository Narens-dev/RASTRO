...
- **RASTRO adopted this starter for its frontend** — cloned into `web/` inside
  the existing RASTRO repo (a Node/Express backend at repo root implementing a
  6-source government-data risk-evidence engine, contract tracking, and a pyme
  opportunities module). All four product views built: `home` (hero + search +
  mode cards + evidence-level band), `expediente` (risk dossier), `contrato`
  (contract tracking with a plazo-vs-pago signal), `oportunidades` (filterable
  licitaciones grid). New `app/api/**` routes proxy to the Express API via a
  shared server-only client (`src/lib/rastro-api.ts`); `expediente`/`contrato`
  fetch their primary data directly in the Server Component instead (no
  client-side loading spinner on first paint). Brand tokens (emerald/gold/ink,
  Manrope+Inter) replace the placeholder palette; dual-tone dark/light sections
  replace `prefers-color-scheme`. `<Cookie>` left unmounted (product sets no
  cookies). New Tier-3 token `--container-page`. See [[decisions-log]] ADR-0018.
