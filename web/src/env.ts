...
const serverSchema = z.object({
  ...
  /**
   * Base URL of the RASTRO Express API (the existing backend — search,
   * scoreEngine, contract tracking, opportunities, AI summary). All
   * `app/api/**` route handlers proxy to it server-side; the browser never
   * calls it directly. Defaults to the local dev server.
   */
  RASTRO_API_URL: z.url().default("http://localhost:3000"),
});
...
export function getServerEnv() {
  cachedServerEnv ??= serverSchema.parse({
    ...
    RASTRO_API_URL: process.env.RASTRO_API_URL || undefined,
  });
